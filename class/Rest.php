...
class Rest{
    ...
    private $user  = 'root';
    private $password   = "fiat122p";
    private $database  = "test_api";      
    private $empTable = 'tbl_emp';
		private $polutionTable = 'tbl_polution_norm';
	...
	public function getEmployee($empId) {		
		...
		$empQuery = "
			SELECT id, name, surname, address, age
	...
	public function getPollution($pollId) {		
		...
		$empQuery = "
			SELECT id, pollution_name, pollution_max_index, pollution_acceptable  
			FROM ".$this->polutionTable." $sqlQuery
			ORDER BY id DESC";	
		$resultData = mysqli_query($this->dbConnect, $empQuery);
		$empData = array();
		while( $empRecord = mysqli_fetch_assoc($resultData) ) {
	...
	function insertEmployee($empData){ 		
		...
		$empSurname=$empData["empSurname"];
		$empAddress=$empData["empAddress"];		
		$empDesignation=$empData["empDesignation"];
		$empQuery="
			INSERT INTO ".$this->empTable." 
			SET name='".$empName."', age='".$empAge."', surname='".$empSurname."', address='".$empAddress."', designation='".$empDesignation."'";
